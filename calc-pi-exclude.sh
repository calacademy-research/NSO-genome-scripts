...
cut -f10,11 $1 | awk '
BEGIN{
   nd["0/0:0/0"] = 0.0; nd["0/1:0/0"] = 0.5; nd["1/1:0/0"] = 1.0;
   nd["0/0:0/1"] = 0.5; nd["0/1:0/1"] = 0.5; nd["1/1:0/1"] = 0.5;
   nd["0/0:1/1"] = 1.0; nd["0/1:1/1"] = 0.5; nd["1/1:1/1"] = 0.0;
}

{
  ...
  if (totcov < 167 && a[1] != "./." && b[1] != "./." && b[1] != "1/1") {
     rnd = a[1]":"b[1]; val=nd[rnd]; totval += val;
     if (val==0) {totzero+=1}
